...
def load_env_config(env_name):
    ...
    config_path = os.path.join(config_dir, f"{env_name}_reporting.json")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python reporting.py <environment>")
        sys.exit(1)
    ...
    print(f"Deploying Reporting workspace → {workspace_id}")
